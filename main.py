...
async def callback_flat_button(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id,"Введите ссылку на Циан",reply_markup=keyboard_stop)
    await Flats.cian.set()
@dp.message_handler(state=Flats.cian)
async def get_cian_url(msg = types.Message,state= FSMContext):
    request=msg.text
    if request != '/stop' and request != 'Отмена':
        await Flats.next()
        await state.update_data(cian_url=request)
        await bot.send_message(msg.from_user.id,'Введите ссылку на Авито',reply_markup=ReplyKeyboardRemove())
    else:
        await bot.send_message(msg.from_user.id, 'Действие отменено',reply_markup=keyboard)
        await state.finish()
@dp.message_handler(state=Flats.avito)
...
async def run_cancel_parser(msg=types.Message,state=FSMContext):
    if msg.text=='Отмена':
        ...
        await state.finish()
        task1=asyncio.create_task(cian_parcer(user_data['cian_url'], msg.from_user.id, bd[msg.from_user.id],int(msg.text)))
        task2=asyncio.create_task(avito_parcer(user_data['avito_url'], msg.from_user.id, bd[msg.from_user.id],int(msg.text)))
        await asyncio.gather(task1,task2)
        #await cian_parcer(user_data['cian_url'], msg.from_user.id, bd[msg.from_user.id],int(msg.text))
